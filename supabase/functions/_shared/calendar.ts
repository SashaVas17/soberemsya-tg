...
export async function isCalendarTicketValid(
  eventId: string,
  expires: number,
  signature: string,
  secret: string,
  ...
  if (!secret || !Number.isInteger(expires) || expires <= nowSeconds) return false;
  return constantTimeEqual(signature, await signCalendarTicket(eventId, expires, secret));
}
...
export function buildIcs(input: {
  ...
  return [
    ...
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
    `UID:${escapeIcs(input.id)}@soberemsya`,
    `DTSTAMP:${icsDate(now)}`,
