...
function Manage({ eventId, navigate }: { eventId: string; navigate: Navigate }) {
  ...
  return <main><Header navigate={navigate} /><PageIntro eyebrow="Управление" title={event.title} compact><div className="manage-summary"><StatusBadge status={event.status} /><span>{plural(event.participants.length, "ответ", "ответа", "ответов")}</span><span>{formatDateRange(event.timeOptions) ?? "Время не выбрано"}</span><span>{plural(event.placeOptions.length, "место", "места", "мест")}</span></div></PageIntro><section className="manage-page"><Refresh refreshing={state.refreshing} lastUpdated={state.lastUpdated} onClick={() => void state.load()} />
    <section className="panel result-overview"><h2>По голосованию</h2>{event.timeOptions.length ? <><h3>Когда</h3>{event.timeOptions.map((item) => <VoteBar key={item.id} label={formatSlot(item.startsAt)} count={item.availableCount} max={maxTime} leader={item.availableCount > 0 && item.availableCount === maxTime} />)}</> : <EmptyState title="Нет вариантов времени" text="Добавьте хотя бы один вариант." />}{event.placeOptions.length > 0 && <><h3>Где</h3>{event.placeOptions.map((item) => <VoteBar key={item.id} label={item.title} count={item.placeVoteCount ?? 0} max={maxPlace} leader={(item.placeVoteCount ?? 0) > 0 && item.placeVoteCount === maxPlace} />)}</>}{areas.length > 0 && <p className="area-summary">{areas.length > 1 ? "Нет единого района: " : "Лидирующий район: "}{areas.map(([area, count]) => `${area} (${count})`).join(", ")}</p>}</section>
